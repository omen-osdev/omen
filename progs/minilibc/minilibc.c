...
int sys_ioctl(int fd, unsigned long request, void * arg) {
    return (int)syscall(16, (uint64_t)fd, (uint64_t)request, (uint64_t)arg, 0, 0, 0);
}
...
int sys_seek(int fd, int offset, int whence) {
    return (int)syscall(8, (uint64_t)fd, (uint64_t)offset, (uint64_t)whence, 0, 0, 0);
}
...
int sys_msync(void * addr, size_t length, int flags) {
    return (int)syscall(26, (uint64_t)addr, (uint64_t)length, (uint64_t)flags, 0, 0, 0);
}

int sys_nanosleep(struct timespec * req, struct timespec * rem) {
    return (int)syscall(35, (uint64_t)req, (uint64_t)rem, 0, 0, 0, 0);
}

int sys_dup(int fd) {
    return (int)syscall(32, (uint64_t)fd, 0, 0, 0, 0, 0);
}
int sys_dup2(int oldfd, int newfd) {
    return (int)syscall(33, (uint64_t)oldfd, (uint64_t)newfd, 0, 0, 0, 0);
}
...
int sys_sigaction(int signum, struct sigaction * act, struct sigaction * oldact) {
    ...
    *(void**) &(act->sa_restorer) = get_vdso_signal_trampoline();
    return (int)syscall(13, (uint64_t)signum, (uint64_t)act, (uint64_t)oldact, 0, 0, 0);
}

int sys_sigsuspend(sigset_t* sigsuspend_mask, const sigset_t *mask) {
    return (int)syscall(130, (uint64_t)sigsuspend_mask, (uint64_t)mask, 0, 0, 0, 0);
}

int sys_sigprocmask(int how, const sigset_t *set, sigset_t *oldset) {
    return (int)syscall(14, (uint64_t)how, (uint64_t)set, (uint64_t)oldset, 0, 0, 0);
}

int sys_sigpending(sigset_t *set) {
    return (int)syscall(127, (uint64_t)set, 0, 0, 0, 0, 0);
}

int sys_kill(int pid, int sig) {
    return (int)syscall(62, (uint64_t)pid, (uint64_t)sig, 0, 0, 0, 0);
}

int sys_sigaltstack(const stack_t *ss, stack_t *oss) {
    return (int)syscall(131, (uint64_t)ss, (uint64_t)oss, 0, 0, 0, 0);
}
...
int sys_gettimeofday(struct timeval *tv, struct timezone *tz) {
    return (int)syscall(96, (uint64_t)tv, (uint64_t)tz, 0, 0, 0, 0);
}

int sys_arch_prctl(int code, unsigned long *addr) {
    return (int)syscall(158, (uint64_t)code, (uint64_t)addr, 0, 0, 0, 0);
}
...
int sys_clock_settime(int clock_id, const struct timespec *tp) {
    return (int)syscall(227, (uint64_t)clock_id, (uint64_t)tp, 0, 0, 0, 0);
}

int sys_clock_gettime(int clock_id, struct timespec *tp) {
    return (int)syscall(228, (uint64_t)clock_id, (uint64_t)tp, 0, 0, 0, 0);
}

int sys_clock_getres(int clock_id, struct timespec *tp) {
    return (int)syscall(229, (uint64_t)clock_id, (uint64_t)tp, 0, 0, 0, 0);
}

int sys_waitpid(int pid, int *status, int options) {
    return (int)syscall(61, (uint64_t)pid, (uint64_t)status, (uint64_t)options, 0, 0, 0);
}
