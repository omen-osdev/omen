#ifndef _CONFIG_H
#define _CONFIG_H

#define ARCH x86_64

#define PAGE_SIZE 0x1000

#define PREEMPTION_TICKS 0x50 //0 TO DISABLE PREEMPTION

#define KERNEL_STACK_SIZE 0x30000
#define PROCESS_STACK_SIZE 0x30000
#define PROCESS_STARTUP_RFLAGS 0x202

#define DSHELL_ECHO 1

#define BOOTLOADER_USE_LIMINE

#define MAX_CPUS 256
#define MAX_NUMA_NODES 16

#define MAX_PROCESSES 256

//Per type of device/driver
#define MAX_DEVICES 256
#define MAX_DEVICE_DRIVERS 256
#define DEVICE_NAME_MAX_SIZE 256

//The debugger will work with a circular buffer
#define DEBUG_MESSAGE_BUFFER 0x1000
#endif