                ...
                case 0x06: {
                    switch (pci->prog_if){
                        case 0x01:
                            ...
                            break;
                        default:
                            DBG_WARN("[PCI] Unknown AHCI interface: %x\n", pci->prog_if);
                    }
                    break;
                }
            }
...
void enumerate_function(uint64_t device_address, uint64_t bus, uint64_t device, uint64_t function) {
    ...
    DBG_DEBUG("[PCI] Device base address: %x\n", device_base_address);
...
void enumerate_device(uint64_t bus_address, uint64_t device, uint64_t bus) {
    ...
    //DBG_INFO("[PCI] enumerating device %x:%x\n", bus_address, device_address);
...
void enumerate_bus(uint64_t base_address, uint64_t bus) {
    ...
    //if (pci_device_header->device_id == 0x0) {DBG_DEBUG("ig0:%ld|", bus); return;}
    //if (pci_device_header->device_id == 0xFFFF) {DBG_DEBUG("igf:%ld|", bus); return;}
...
void init_pci() {
    ...
    DBG_INFO("[PCI] Registering pci devices... %ld entries\n", entries);
