...
#include <omen/modules/managers/dev/devices.h>
...
char * device_create(void * device_control_structure, const uint8_t major, const uint64_t id) {
    ...
    if (device_control_structure == NULL) {
        RECOVERABLE_ERROR(INVALID_ARGUMENT, "Device control structure not provided");
    }
    ...
    if (id < 0) {
        RECOVERABLE_ERROR(INVALID_ARGUMENT, "ID too small");
    }
...
driver_t* driver_get(const uint8_t major) {
    if (major < CHAR_MIN_MAJOR) {
        ...
        return &block_device_drivers[major & 0x7f];
    }
}
...
status_t device_identify(const char* device, const char* driver) {
    ...
    if (strcmp(driver->name, driver) != 0) {
        RECOVERABLE_ERROR(INVALID_ARGUMENT, "Driver name %s does not match driver name %s", driver, driver->name);
    }
