...
void pmm_init() {
    if (ready) {
        DBG_WARN("PMM already initialized\n");
        return;
    }
    ...
    if (biggest_index == -1) {
        DBG_ERROR("No usable memory found\n");
        return;
    }
...
void pmm_list_map() {
    ...
    uint64_t entries = get_memory_map_entries();
    DBG_DEBUG("Memory map entries: %d\n", entries);
    for (uint64_t i = 0; i < entries; i++) {
        DBG_DEBUG("Entry %d: base: 0x%x, length: 0x%x, type: %d\n", i, get_memory_map_base(i), get_memory_map_length(i), get_memory_map_type(i));
    }
}
